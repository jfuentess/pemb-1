...
vector<char> dfs_spanning_tree_propio(unsigned int init, unsigned int
    ...
    bool *visited_edge = new bool[m]();
    bool *visited_region = new bool[total](); 
    bool *bits = new bool[n*niveles]();  
    for(int i = 0; i < n ; i++)visited_vertex[i] = no_visited;
    for(int i = 0; i < m ; i++)visited_edge[i] = no_traversed;
    for(int i = 0; i < total ; i++)visited_region[i] = no_visited;
    vector <char> secuencia; 
    stack <unsigned int> o,e;
    visited_vertex[init] = visitado;
   // visited_region[init] = visitado;
    for(int i = 0; i < niveles; i++){
     int regionaux =  jerarquias[init*niveles + i];
     visited_region[regionaux + limites[i]] = visitado;
    }

    for(unsigned int i = this->V[init].getFirst(); i <= this->V[init].getLast(); i++) o.push(i);
    while(!o.empty()) {
      unsigned int curr = o.top(); o.pop();


      bool auxcheck = visited_vertex[this->E[curr].getTgt()] , auxcheckr = false;
      for(int i = 1; i < niveles; i++){

        int regionA =  jerarquias[this->E[curr].getSrc()*niveles + i];
        int regionB = jerarquias[this->E[curr].getTgt()*niveles + i];
        if(visited_region[regionB + limites[i]] == no_visited || regionA == regionB){
          auxcheckr = true;
          break;
        }
      }

      if(auxcheck && auxcheckr && visited_edge[curr] == no_visited){ //cumple con condicion para visitar vertices y es primera vez que se visita
        secuencia.push_back('(');
        visited_edge[curr] = traversed;
        visited_vertex[this->E[curr].getTgt()] = visitado;
        e.push(curr);
        unsigned int nodotgt = this->E[curr].getTgt();
        for(unsigned int i = this->V[nodotgt].getFirst(); i <= this->V[nodotgt].getLast(); i++){
          o.push(i);
        }
        for(unsigned int i = 0; i < niveles; i++){
          unsigned int p =  jerarquias[nodotgt *niveles + i] + limites[i];
          if(visited_region[p] == no_visited){
              visited_region[p] = visitado;
              bits[i*n + nodotgt] = true;
          }
        }
      ...
      else if( !(auxcheck && auxcheckr) &&  visited_edge[curr] == no_visited ){
                secuencia.push_back('[');
                visited_edge[curr] = traversed;
      }
      ...
      else if((auxcheck && auxcheckr) &&  visited_edge[curr] == visitado ){
                secuencia.push_back(')');
      }
