...
auxiliar dfs_spanning_tree_propio(unsigned int init, unsigned int
        *jerarquias, unsigned int *limites, unsigned int niveles,unsigned int total, unsigned int totale) {
    ...
    bool *cbits = new bool[totale*niveles*2](); 
	...
	cout<< "TAMAÑO NUEVO = " <<totale*niveles*2 <<endl;
...
bool auxcheckraux = true;
      for(int j = i+1; j < niveles; j++){
        ...
        if(   !(auxcheckraux && visited_region[p] == no_visited) && p != q && mapa[make_pair(p,q)]==0 ){
          ...
          secuencia[i].push_back('[');
	if(poscbit == 1125)cout << "IMPRIMO EN NIVEL : " << i << " CON poscbit " << poscbit  <<  " " <<  auxcheckraux  << " " <<  visited_region[p] << " " <<  mapa[make_pair(p,q)]<<endl;
          cbits[(i*totale*2) + poscbit] = true;
           if(i == 1)a2++;
          mapa[make_pair(p,q)]++;
          mapa[make_pair(q,p)]++;
          }
...
//cuarto caso, arista ya fue visitada 1 vez y esta no pertenece al st.
        ...
        unsigned int p =  jerarquias[this->E[curr].getTgt() *niveles + i] + limites[i];
        unsigned int q =  jerarquias[this->E[curr].getSrc() *niveles + i] + limites[i];
//if(i == 1 && poscbit == 1125)         if(poscbit == 1125)cout << "IMPRIMO EN NIVEL : " << i << " CON poscbit " << poscbit  <<  " " << p  << " " <<  q << " " <<  mapa[make_pair(p,q)]
// <<  " " <<  (jerarquias[this->E[curr].getSrc() *niveles + (i-1)] + limites[i-1]) <<" " <<prueba2[i][make_pair(p,q)]-1 << " " << (jerarquias[this->E[curr].getTgt() *niveles + (i-1)] + limites[i-1])<< endl;
        if(p != q && mapa[make_pair(p,q)]==1 && i != 0 && (jerarquias[this->E[curr].getSrc() *niveles + (i-1)] + limites[i-1]) == prueba2[i][make_pair(p,q)]-1 && i != 0
          ...
          secuencia[i].push_back(']');
 //if(i == 1 &&  poscbit < 21)cout << "IMPRIMO2: " << poscbit << " " << posbit  << endl;  
 //       if(poscbit == 1125)cout << "IMPRIMO EN NIVEL : " << i << " CON poscbit " << poscbit    << " " <<  mapa[make_pair(p,q)]<<endl;
...
//for(int j = 0; j < secuencia.size();j++)        for(int i = 0; i <secuencia[j].size();i++)printf("%c",secuencia[j][i]);
    return auxiliar(niveles,secuencia,bits,original,reverso,cierres,cbits);
  }
